 ...
 */
namespace Vass\Custom\Model\Import\Product;
...
class Validator extends \Magento\CatalogImportExport\Model\Import\Product\Validator
{
    ...
    /**
     * @var \Vass\Custom\Helper\Data
     */
    private $helperCustom;
   

    private $countSpecialPriceCero;
    private $rowSpecialPriceCero;
    private $messagesSuggestionAttr = [];
     ...
     * @return bool
     */
    private function validateOption($attrCode, $possibleOptions, $value, $rowNum = null)
    {   
        if (!isset($possibleOptions[strtolower($value)])) {
            $valueArray= str_split(strtolower($value));
            foreach($possibleOptions as $key => $valueAttr){
                $keyArray= str_split(strtolower($key));
                $matchs = array_intersect($keyArray, $valueArray);
                $lengthValue = strlen($key)/2;
                if(count($matchs) > $lengthValue ){
                    $this->messagesSuggestionAttr[$attrCode][$key]["writes"][strtolower($value)] = $value;
                    $this->messagesSuggestionAttr[$attrCode][$key]["rows"][$rowNum] = $rowNum;
                }
            }   
            $this->_addMessages(
                [
    ...
    public function isAttributeValid($attrCode, array $attrParams, array $rowData, $rowNum = '')
    {
        ...
        if($attrCode == "price" && $rowData["price"] <= 0 && $rowData["product_type"] != "configurable") {
            $valid = false;
            $this->_addMessages(
                [
                    'Please make sure attribute "price" value is greater than zero'
                ]
            );
            return $valid;
        }
        if($attrCode == "sku"){
            $product = $this->helperCustom->getProductBySku($rowData["sku"]);
            if($product){
                $valid = false;
                if($product->getUpdatedAt() > $rowData["updated_at"]){
 
                    $this->_addMessages(
                        [
                            sprintf(
                                'You are trying to import outdated products'
                                ,
                                $attrCode
                            )
                        ]
                ...
                if($rowData["special_price"] == 0 && $product->getSpecialPrice() > 0){
                    if(is_numeric($rowNum)){
                        $valid = false;
                        $this->countSpecialPriceCero ++;
                        $rowNum = $rowNum + 2;
                        if($this->rowSpecialPriceCero){
                            $this->rowSpecialPriceCero .=  ",".$rowNum;
                        }else{
                            $this->rowSpecialPriceCero .=  $rowNum;
                        }
        ...
        switch ($attrParams['type']) {
            ...
            case 'select':
            case 'boolean':
                $valid = $this->validateOption($attrCode, $attrParams['options'], $rowData[$attrCode], $rowNum);
                break;
            case 'multiselect':
                $values = $this->context->parseMultiselectValues($rowData[$attrCode]);
                foreach ($values as $value) {
                    $valid = $this->validateOption($attrCode, $attrParams['options'], $value, $rowNum);
                    if (!$valid) {
                        break;
                    }
                }
    ...
    public function getProductoSpecialPriceZero(){
        return $this->countSpecialPriceCero;
    }

    public function getRowSpecialPriceCero(){
        return $this->rowSpecialPriceCero;
    }

    public function getMessagesSuggestionAttr(){
        return $this->messagesSuggestionAttr;
    }
}
