...
class Complete extends \Magento\Framework\App\Action\Action
{
	...
	public function execute() {
		...
		if ($signatureOrder !== $signaturePayuLatam) {
			...
			$resultRedirect->setPath('checkout/compranoexitosa');
			return $resultRedirect;
		}
		$payment = $order->getPayment();
		$statuses = $this->_helperData->getOrderStates();
		$approvedOrder = \Magento\Sales\Model\Order::STATE_PROCESSING;
		...
		$pathRedirect = 'checkout/compraexitosa';
		if ($order->getState() === $pendingOrder && $statusTransaction === '7') {
			...
			$order->cancel()->save();
			$pathRedirect = 'checkout/compranoexitosa';
			...
			$order->setState($approvedOrder)->setStatus($status);
			...
			$this->_dbTransaction
				...
				->save();
			$order->addStatusHistoryComment(__('Invoice #%1', $invoice->getId()))->setIsCustomerNotified(true);
